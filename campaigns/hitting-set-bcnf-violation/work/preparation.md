...
Date: 2026-09-23. The fixed corpus has 121 distinct source instances: 15 designed edge cases and 106 seeded random cases (accepted seeds 0–136). Universe size is 0–6, family size 0–8, and `k` ranges from 0 to 7, including `k > n`. There are 44 solvable and 77 unsolvable cases. The generator recomputes each expected *existence* answer by enumerating subsets. The self-test reconstructs every seeded case, compares all labels against Z3 5.1.0, and validates returned witnesses from the source definition. This is an existence label, not a fixed chosen witness.
